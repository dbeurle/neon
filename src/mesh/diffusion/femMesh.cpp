...
//     : nodal_coordinates(std::make_shared<NodalCoordinates>(basic_mesh.coordinates()))
// {
...
//     {
//         submeshes.emplace_back(material_data, simulation_data, nodal_coordinates, submesh);
//     }
//
...
//     {
//         for (auto & [ name, boundaries ] : dirichlet_bcs)
//         {
...
//                 }
//
//                 auto const offset_dirichlet_dofs = view::transform(dirichlet_dofs,
//                                                                    [&](auto const& dof) {
//                                                                        return dof + dof_offset;
//                                                                    });
//                 dirichlet_bcs[boundary_name].emplace_back(offset_dirichlet_dofs,
...
//                 auto const& dof_offset = dof_table.find(name)->second;
//
//                 flux_bcs[boundary_name].emplace_back(nodal_coordinates,
...
//         else
//         {
//             throw std::runtime_error("BoundaryCondition \"" + boundary_type + "\" is not
//             recognised");
//         }
//     }
// }
...
//         auto const& boundary_name = boundary["Name"].asString();
//
//         if (boundary["Type"].asString() == "Temperature"
//             || boundary["Type"].asString() == "Concentration")
//         {
//             for (auto const& name : boundary["Values"].getMemberNames())
//             {
//                 for (auto& dirichlet_boundary : dirichlet_bcs[boundary_name])
//                 {
