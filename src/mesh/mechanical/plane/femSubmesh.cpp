...
matrix femSubmesh::geometric_tangent_stiffness(matrix const& x, int32 const element) const
{
    ...
    matrix const kgeo = sf->quadrature()
                            .integrate(matrix::Zero(n, n).eval(),
                                       [&](auto const& femval, auto const& l) -> matrix {
                                           ...
                                           matrix2 const cauchy = cauchy_list[view(element, l)];
...
matrix femSubmesh::material_tangent_stiffness(matrix const& x, int32 const element) const
{
...
vector femSubmesh::internal_nodal_force(matrix const& x, int32 const element) const
{
