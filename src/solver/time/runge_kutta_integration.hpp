...
auto runge_kutta_second_order(functor&& f)
{
    return [f](auto const t, auto const y, auto const dt) {
        ...
        auto const dy1 = f(t, y);
        auto const dy2 = f(t + dt, y + dy1);
        return dt * (dy1 + dy2) / 2.0;
    };
}
...
auto runge_kutta_fourth_order(functor&& f)
{
    return [f](auto const t, auto const y, auto const dt) {
        ...
        auto const dy1 = f(t, y);
        auto const dy2 = f(t + dt / 2.0, y + dy1 / 2.0);
        auto const dy3 = f(t + dt / 2.0, y + dy2 / 2.0);
        auto const dy4 = f(t + dt, y + dy3);
        return dt * (dy1 + 2.0 * dy2 + 2.0 * dy3 + dy4) / 6.0;
    };
}
}
