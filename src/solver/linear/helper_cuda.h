...
// refer the CUDA examples for examples of the needed CUDA headers, which may change
// depending on which CUDA functions are used.
...
inline int _ConvertSMVer2Cores(int major, int minor)
{
    ...
    typedef struct
    {
        int SM; // 0xMm (hexidecimal notation), M = SM Major version, and m = SM minor
                // version
        int Cores;
    } sSMtoCores;
...
inline int gpuGetMaxGflopsDeviceId()
{
    ...
    while (current_device < device_count)
    {
        ...
        // If this GPU is not running on Compute Mode prohibited, then we can add it to
        // the list
        if (deviceProp.computeMode != cudaComputeModeProhibited)
        {
    ...
    {
        fprintf(stderr,
                "gpuGetMaxGflopsDeviceId() CUDA error: all devices have compute mode "
                "prohibited.\n");
        exit(EXIT_FAILURE);
    }
